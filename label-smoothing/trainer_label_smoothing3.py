"""
[이게 제일 new !]
- 2랑 seed 설정할 수 있는게 다르다.

label smoothing - test3

- transfer_learning + LR_scheduler + labelsmoothing
...
from model_archive.ResNet_Dropout import ResNet_dropout, Config
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default='resnet50')     # resnet18, 34, 50, 101, 152.
    ...
    parser.add_argument("--label_smoothing_const", type=float, default=None)   # normally 0.1 I guess.
    ...
    parser.add_argument("--dropout", type=str, default='no')
    parser.add_argument("--seed", type=int, default=42)
    # parser.add_argument("--L2", type=str, default='no')   # TODO
    ...
    print(time.strftime('%c', time.localtime(time.time()))) # time for tensorboard logs...
    ...
    if args.pretrained!='yes':
        # make new model.
        print('[i] train newly initialized model!')
        config = Config()
        resnet_config = config.get_resnet_config(model_name = args.model)
        OUTPUT_DIM = len(test_data.classes)
        if args.dropout=='yes':
            print("[i] dropout is applied")
            model = ResNet_dropout(resnet_config, OUTPUT_DIM)
        else:
    ...
    if args.label_smoothing_const!=None:
        print(f'[i] label smoothing smooth_const {args.label_smoothing_const}')
        criterion = LabelSmoothingLoss(classes=len(train_iterator), smoothing=args.label_smoothing_const, dim=-1)
    # else:
    ...
    for epoch in range(args.epochs):
        ...
        writer.add_scalar("acc@1/train", train_acc_1, epoch)
        writer.add_scalar("acc@1/valid", valid_acc_1, epoch)
        writer.add_scalar("acc@5/train", train_acc_5, epoch)
        writer.add_scalar("acc@5/valid", valid_acc_5, epoch)
        if args.dropout=='yes':
            writer.add_scalar("dropout testing model", resnet_with_dropout_only, epoch)
